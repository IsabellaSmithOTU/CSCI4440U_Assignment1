...
#command 
#python eval_A2C.py --model_path ./models/A2C_model
...
def main():
    ...
    p.add_argument("--model_path", type=str, default="./models/a2c_explorer") #might need to fix directory
    ...
    #p.add_argument("--csv_out", type=str, default="./logs/eval_metrics.csv") mistake, causing overwrites bruh
...
if __name__ == "__main__":
    ...
    while not (done or trunc):
        ...
        if render:
            ...
            #print every step or every 10 steps, to show progress of eval
            if steps % 10 == 0 or done:
                print(f"step {steps}, reward{ep_reward:.2f}")

    # Episode-level metrics from env info
    success = int(done and info.get("goal_reached", False))
    truncated_flag = int(trunc)
    visited = info.get("visited_cells", 0)
    total = info.get("total_cells",1)
    coverage = float(visited)/float(total) if total>0 else 0.0
...
def main():
    p = argparse.ArgumentParser()
    p.add_argument("--model_path", type=str, default="./models/a2c_model") #might need to fix directory
    p.add_argument("--maze_path", type=str, default="../envs/maze_samples/maze2d_5x5.npy")
    p.add_argument("--episodes", type=int, default=10)
    p.add_argument("--render", type=int, default=0)
    p.add_argument("--reward_mode", type=str, default="explorer", choices=["explorer", "survivor"]) #should i keep both option, check later
...
#TODO: add the file saving code 
    if not os.path.exists(args.model_path + ".zip"):
        raise FileNotFoundError(f"Model not found: {args.model_path}.zip")
    model_name = os.path.splitext(os.path.basename(args.model_path))[0] #saving csvs seperately
    #os.makedirs(os.path.dirname(args.csv_out), exist_ok=True)
    model = A2C.load(args.model_path)
    csv_file = os.path.join("logs", f"eval_metrics_{model_name}.csv")
