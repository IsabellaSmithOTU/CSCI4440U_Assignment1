...
class MazeView2D:
    ...
    def move_robot(self, dir):
        ...
        if self.__maze.is_open(self.__robot, dir):
            ...
            if self.maze.is_portal(self.robot):
                ...
                #tracking cells visited by blue ball/agent/robot
            
            #CONFUSed, rechecking code because tracking not happening and i think ive done in 2 files
            #
            # self.visited_cells.add(tuple(self.__robot)) #its still white the variable, todo fix later
    ...
    def reset_robot(self):
        ...
        self.visited_cells = {tuple(self.__robot)} #resetting visited cells by robot/ blue ball/agent
        self.__draw_robot(transparency=255)
...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10,10), has_loops=True, num_portals=0):
        ...
        #adding cell tracking of blue ball/robot
        self.visited_cells= set()  #confusions bw visite_new and visited_new_cells, so changing, and TODO: review after to make sure all files same var name
       #(INCORRECT ._) self.visited_cells.add(tuple(self._robot)) #im gonna remove this if i can figure out way to use num cells
